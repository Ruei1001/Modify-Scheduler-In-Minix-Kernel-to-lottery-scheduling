...
void proc_init(void)
{
	...
	for (rp = BEG_PROC_ADDR, i = -NR_TASKS; rp < END_PROC_ADDR; ++rp, ++i) {
		...
		/*-------------------------------------------------------------------------------------*/
		rp->p_ticketNums = 5;       /*initial the ticket num a proc pocess*/
		/*-------------------------------------------------------------------------------------*/
 ...
 *				setProcTicket					     * 
 *===========================================================================*/
int setProcTicket(int num, struct proc * pc)
{
	/*this is a function to set the ticket umber of a process input num is the number of
	 *ticket decides to add for that process*/
	int add;
	add = pc->p_ticketNums + num > 10 ? 10 - pc->p_ticketNums : num;
	add = pc-> p_ticketNums + num < 1 ? 1 - pc->p_ticketNums : num;
	pc->p_ticketNums += add;
	return add; 
}
 ...
 */
  ...
  setProcTicket(-1, rp);  
  	for (q=0; q < NR_SCHED_QUEUES; q++)
	{
		...
		printf("Priority level %d : %d process", q, count);
		count = 0;
	}
  /*-------------------------------------------------------------------------------------*/
 ...
 */
  ...
  int nTickets = 0;     /*to record how many ticket total */
  int jackpot;
  char old_priority;
  int flag = 0;
  ...
  for (q=0; q < 6; q++) {	
	...
	srandom(time(NULL));
	jackpot = nTickets ? random() % nTickets : 0;
	...
	for (q=6; q < NR_SCHED_QUEUES; q++)
	{
		...
		while (1)
		{
			if(rp -> p_rts_flags == 0)
			{
				old_priority = rp->p_priority;
				if(jackpot >= 0)
				{
					jackpot -= rp->p_ticketNums;
					if(jackpot < 0)
					{
						flag = 1;
						assert(proc_is_runnable(rp));
...
{
	/*-------------------------------------------------------------------------------------*/
	setProcTicket(-1, p);       /* Run out of time quantum take away 1 ticket*/
	/*-------------------------------------------------------------------------------------*/
